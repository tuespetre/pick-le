...
(function () {
    ...
    function filterItems(control, filter) {
        ...
        var itemToHighlight = getFirstCheckedItem(control);
    ...
    function focusCollapsedTarget(control) {
        var target = control.querySelector(SELECTOR.PICK_LE_FOCUS) || control;
        target.focus();
    }
    ...
    function setDisplayText(control, option) {
        ...
        var displayTextElement = control.querySelector(SELECTOR.PICK_LE_TEXT);
    ...
    function setTitleText(control, text) {
        var titleTextElement = control.shadowRoot.querySelector(SELECTOR.PRIVATE_TITLE);
    ...
    function handleKeyNavigation(control, event, getSibling) {
        var highlightedItem = getFirstHighlightedItem(control) || getFirstCheckedItem(control);

        if (!highlightedItem) {
            var firstVisibleItem = getFirstVisibleItem(control);
    ...
    var pickle = function (_HTMLElement) {
        ...
        function pickle() {
            ...
            var _this = _possibleConstructorReturn(this, (pickle.__proto__ || Object.getPrototypeOf(pickle)).call(this));

            _this.attachShadow({ mode: 'open' });
            _this.shadowRoot.appendChild(TEMPLATE_MAIN.cloneNode(true));
            ...
            _this.addEventListener('keydown', handleKeydown.bind(_this));

            _this.shadowRoot.querySelector(SELECTOR.POPUP).addEventListener('click', function (event) {
                event.stopPropagation();
            });

            _this.shadowRoot.querySelector(SELECTOR.CLOSE).addEventListener('click', function (event) {
                _this.expanded = false;
                focusCollapsedTarget(_this);
            });
            ...
            filterInput.addEventListener('input', function (event) {
                var oldValue = filterInput.getAttribute(ATTR.DATA_VALUE);
                var newValue = event.target.value;

                if (newValue !== oldValue) {
                    filterInput.setAttribute(ATTR.DATA_VALUE, newValue);
                    filterItems(_this, newValue);
                }
            });

            filterInput.addEventListener('keydown', handleKeydown.bind(_this));
            return _this;
        }
        ...
        _createClass(pickle, [{
            ...
            set: function set(value) {
                ...
                if (this.expanded === expanded) {
                    ...
                    var firstChecked = getFirstCheckedItem(this);
                    if (firstChecked) {
