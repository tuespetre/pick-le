...
(function () {
    ...
    var SHADOWDOM_POLYFILLED = window.shadowDomPolyfilled;
    ...
    if (!SHADOWDOM_POLYFILLED) {
        ...
        var transform = function transform(node) {
            return Array.prototype.forEach.call(node.querySelectorAll('*'), function (elem) {
                return elem.classList.add(CLASS.SCOPE);
            });
        };
    ...
    var internals = Object.freeze({
        ...
        create_option: function create_option(option, newNode, addHref) {
            var opt = newNode;
        ...
        */
        each_option: function each_option(func) {
        ...
        scroll_option_into_view: function scroll_option_into_view(option) {
            ...
            var menu = this.shadowRoot.querySelector(SELECTOR.LIST),
                v_rect = undefined,
        ...
        apply_filter: function apply_filter(value) {
            var _this = this;

            var allWereHidden = true,
                highlightedOne = false,
                firstVisible = undefined,
                applyHighlight = function applyHighlight(option, highlighted) {
                return internals.apply_highlight.call(_this, option, highlighted);
            },
                getFirstVisible = function getFirstVisible() {
                return internals.get_first_visible_prefer_selected.call(_this);
            };

            internals.each_option.call(this, function (option) {
                var match = internals.match_filter(option, value),
            ...
            !highlightedOne && (firstVisible = getFirstVisible()) && applyHighlight(firstVisible, true);
        },

        match_filter: function match_filter(option, filter) {
        ...
        toggle_option: function toggle_option(option) {
            var _this2 = this;

            var disabled = option.getAttribute(ATTR.ARIA_DISABLED) === TRUE_STRING,
                list = this.list || {},
            ...
            internals.each_option.call(this, function (opt) {
                var index = opt.getAttribute(ATTR.DATA_INDEX),
                    listOption = list[opt.getAttribute(ATTR.DATA_INDEX)],
                    identical = opt.id === option.id,
                    selected = multiple ? identical ? !listOption.selected : listOption.selected : identical;
                ...
                internals.apply_highlight.call(_this2, opt, identical);
        ...
        get_focus_target: function get_focus_target() {
            return this.filterable ? this.shadowRoot.querySelector(SELECTOR.FILTER_INPUT) : this.shadowRoot.querySelector(SELECTOR.POPUP);
        },

        focus_collapsed_target: function focus_collapsed_target() {
            var target = this.querySelector(SELECTOR.PICK_LE_FOCUS) || this;

            target.focus();
        },

        get_highlighted_option: function get_highlighted_option() {
            var id = this.getAttribute(ATTR.ARIA_ACTIVEDESCENDANT);

            return this.shadowRoot.getElementById(id);
        },

        set_display_text: function set_display_text(option) {
            var elem = this.querySelector(SELECTOR.PICK_LE_TEXT),
                value = option ? option.getAttribute(ATTR.DATA_LABEL) || option.getAttribute(ATTR.DATA_TEXT) : null;

            if (elem) elem.textContent = value;
        },

        respond_xs_media_query: function respond_xs_media_query(query) {
            var list = this.shadowRoot.querySelector(SELECTOR.LIST);
        ...
        set_title_text: function set_title_text(text) {
            var elem = this.shadowRoot.querySelector(SELECTOR.PRIVATE_TITLE);

            if (elem) elem.textContent = text;
        },

        get_first_visible_prefer_selected: function get_first_visible_prefer_selected() {
            var list = this.shadowRoot.querySelector(SELECTOR.LIST);

            return list.querySelector(SELECTOR.SELECTED_VISIBLE) || list.querySelector(SELECTOR.VISIBLE);
        }
    ...
    var handlers = Object.freeze({
        ...
        pickle_click: function pickle_click(e) {
        ...
        pickle_keydown: function pickle_keydown(e) {
            switch (KEY_MAP[e.key || e.which || e.keyCode]) {

                case KEY_UP:
                    handlers.key_up_down.call(this, e, function (t) {
        ...
        key_up_down: function key_up_down(e, getSibling) {
            var wasExpanded = this.expanded,
                list = this.querySelector(SELECTOR.LIST),
                target = internals.get_highlighted_option.call(this) || internals.get_first_visible_prefer_selected.call(this),
                original = target;
        ...
        key_escape: function key_escape(e) {
            if (this.expanded) {
                this.expanded = false;
                internals.focus_collapsed_target.call(this);
                e.preventDefault();
        ...
        key_space: function key_space(e) {
            if (!this.expanded) {
                this.expanded = true;
                e.preventDefault();
                e.stopPropagation();
        ...
        key_enter: function key_enter(e) {
            if (!this.expanded) {
                this.expanded = true;
            } else {
        ...
        key_other: function key_other(e) {
            var _this3 = this;

            var filter = function filter() {
                return _this3.shadowRoot.querySelector(SELECTOR.FILTER_INPUT);
            };
        ...
        menu_click: function menu_click(e) {
            e.stopPropagation();
        },

        close_click: function close_click(e) {
            this.expanded = false;
            internals.focus_collapsed_target.call(this);
        },

        filter_input: function filter_input(e) {
        ...
        option_click: function option_click(e) {
            var target = e.target,
                isList = function isList(target) {
                return target.classList && target.classList.contains(CLASS.LIST);
            },
                isContainer = function isContainer(target) {
                return target.classList && target.classList.contains(CLASS.PRIVATE_LIST);
            },
                isOption = function isOption(target) {
                return target.classList && target.classList.contains(CLASS.OPTION);
            };
        ...
        document_click: function document_click(e) {
            var isFromMe = e.composedPath().indexOf(this) !== -1;
    ...
    var pickle = function (_HTMLElement) {
        _inherits(pickle, _HTMLElement);
        ...
        _createClass(pickle, [{
            ...
            value: function connectedCallback() {
                ...
                var handlerMap = [[this, EVENT.CLICK, handlers.pickle_click], [this, EVENT.KEYDOWN, handlers.pickle_keydown], [menu, EVENT.CLICK, handlers.menu_click], [close, EVENT.CLICK, handlers.close_click], [filter, EVENT.INPUT, handlers.filter_input], [filter, EVENT.KEYDOWN, handlers.pickle_keydown], [list, EVENT.CLICK, handlers.option_click], [document, EVENT.CLICK, handlers.document_click]];

                handlerMap.forEach(function (def) {
                    return def[0].addEventListener(def[1], def[2].bind(_this5));
                });
                ...
                // init filterable
                var hidden = this.hasAttribute(ATTR.FILTERABLE) ? FALSE_STRING : TRUE_STRING;
                select(SELECTOR.FILTER).setAttribute(ATTR.ARIA_HIDDEN, hidden);

                // init first selected item
                var firstSelected = list.querySelector(SELECTOR.SELECTED_VISIBLE);
            ...
            set: function set(value) {
                var navigational = value ? true : false;
            ...
            set: function set(list) {
                ...
                var oldContainer = this.shadowRoot.querySelector(SELECTOR.PRIVATE_LIST),
                    newContainer = document.createElement('div'),
                    set = function set(name, value) {
                    return _this6.setAttribute(name, value);
                },
                ...
                if (list instanceof HTMLSelectElement) {
                    ...
                    for (var i = 0; i < options.length; i++) {
                        var option = internals.create_option(options[i], shell.cloneNode(true), addHref);
                        newContainer.appendChild(option);
                    }

                    var selected = newContainer.querySelector(SELECTOR.SELECTED_VISIBLE);
            ...
            set: function set(value) {
                ...
                var current = this.expanded,
                    expanded = value === true,
                    attr = function attr(val) {
                    return _this7.setAttribute(ATTR.ARIA_EXPANDED, val);
                },
                    select = function select(s) {
                    return _this7.shadowRoot.querySelector(s);
                },
                    list = select(SELECTOR.LIST),
                ...
                if (current === expanded) {
                    ...
                    var filter = select(SELECTOR.FILTER_INPUT);
                    ...
                    var active = internals.get_first_visible_prefer_selected.call(this);
